...
*   **Outboard Pontoons**: Dual 6-inch (150mm nominal / $168.3\text{mm}$ OD) SDR-35 PVC pipes, $2.53\text{m}$ length, mounted **touching each other** along the centerline (total width: $336.6\text{mm}$).
...
*   **Vertical Keel Fin**: A single 3-inch ($88.9\text{mm}$ OD) PVC pipe extending centerline from the bottom of the outer pontoons down to the keel bulb (length: $400\text{mm}$). This pipe acts as a rigid structural strut and a watertight conduit for routing motor and battery wiring up to the deck.
*   **Central Keel Bulb**: A $140\text{mm}$ diameter, $900\text{mm}$ long PVC capsule suspended centerline at $z = -450\text{mm}$ below the outer hulls. Features a custom 3D-printed aerodynamic nose cone.
*   **Overall Beam Width**: $1.0\text{m}$ (frame width).
*   **Solar Deck Size**: $1.4\text{m}$ width $\times 2.0\text{m}$ length ($2.8\text{ m}^2$). Made of $4\text{mm}$ Coroplast, overhanging the narrow hulls by $530\text{mm}$ on both sides.
*   **Total Dry Mass**: $\approx 96.5\text{ kg}$ (Payload: $85\text{ kg}$ + Hulls/Frame: $11.5\text{ kg}$).
*   **Draft**: $\approx 61\%$ under full $96.5\text{ kg}$ load.

### 1.2 Self-Righting Moments (SWATH Physics)
*   **Center of Gravity ($G$)**: Housed deep inside the central keel bulb ($z \approx -450\text{mm}$ below the waterline) where the heavy $35\text{ kg}$ battery bank is located.
*   **Center of Buoyancy ($B$)**: Located near the water line ($z \approx -80\text{mm}$).
*   **Self-Righting Capability**: With $G$ slung $370\text{mm}$ below $B$, the vessel has an extremely high righting arm ($GZ$). In the event of a roll past $90^\circ$ or a complete capsize ($180^\circ$), the heavy battery ballast bulb will swing downwards, immediately self-righting the vessel.

---

## 2. Power & Electrical Architecture

*   **Submerged Battery Vault**: The $48\text{V } 115\text{Ah}$ LiFePO4 battery bank ($35\text{ kg}$) is packed inside the watertight central keel bulb. Sinking the battery cells underwater provides excellent passive cooling from the cold sea water.
...
*   **Redundant Charging**: The array is split into two independent $400\text{W}$ charging circuits running to two separate **Victron SmartSolar MPPT 75/15** controllers.
...
*   **Stern-Mounted Keel Thruster**: A single brushless thruster (e.g., BlueRobotics T200 with potted stator and ceramic water-lubricated bearings) bolted directly to the stern transom of the central keel bulb.
    *   *Cruise Speed*: $\approx 3.4\text{ knots}$ at $60\text{W}$ power draw (increased efficiency due to lower drag and inline thrust alignment).
...
*   **Primary Coastal Link**: MeshCore LoRa companion node operating at $915\text{ MHz}$, transmitting diagnostics (`TEL:DIAG`) to onshore repeaters up to $45\text{ NM}$.
...
5.  **Frame and Deck Attachment**: Assemble a simple rectangular frame using 2020 aluminum extrusions ($1.0\text{m} \times 2.0\text{m}$). Secure it to the top of the outboard pontoons using standard brackets. Bolt the $4\text{mm}$ Coroplast deck, mount the 8 flexible solar panels, and route all wiring through glands into the deck-mounted electronics vault.
