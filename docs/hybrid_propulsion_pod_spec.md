...
This document outlines the detailed mechanical, hydrodynamic, and electrical specifications for the **High-Efficiency Hybrid Propulsion Pod** for the Blue-Water Rover ASV. This system combines the extreme durability of seal-less brushless motor technology with the fluid dynamic efficiency of a large-diameter, low-RPM propeller, optimized for sustained cruising in the $3\text{–}5\text{ knots}$ range.

---

## 1. Executive Summary & Design Goals

Standard ROV thrusters (such as the BlueRobotics T200) are optimized for static thrust (bollard pull) and compact packaging, utilizing small, high-RPM propellers ($96\text{ mm}$ diameter). At cruising speeds of $3\text{–}5\text{ knots}$, this results in excessive slip and low efficiency ($\approx 35\text{–}40\%$). 

To achieve multi-day transoceanic crossings on solar power, the propulsion system must operate at peak efficiency. This specification details two hybrid design paths to achieve a target propulsion efficiency of **$60\text{–}70\%$** while maintaining complete waterproofing integrity for a $\approx 100\text{ kg}$ displacement vessel:
...
Where:
*   $T$ = Thrust ($15\text{ N}$ at $3\text{ kts}$, $30\text{ N}$ at $4\text{ kts}$, $70\text{ N}$ at $5\text{ kts}$)
*   $\rho$ = Water density ($1000\text{ kg/m}^3$)
*   $A$ = Propeller swept area ($\frac{\pi D^2}{4}$)
*   $V$ = Cruising speed ($3\text{ kts} \approx 1.54\text{ m/s}$, $4\text{ kts} \approx 2.06\text{ m/s}$, $5\text{ kts} \approx 2.57\text{ m/s}$)

For a **$230\text{ mm}$ ($9.0\text{ inch}$)** diameter propeller compared to a **$96\text{ mm}$ ($3.8\text{ inch}$)** propeller, the swept area increases by **$570\%$**:
*   $A_{96\text{mm}} = 0.0072\text{ m}^2$
*   $A_{230\text{mm}} = 0.0415\text{ m}^2$

At $4\text{ knots}$ and $30\text{ N}$ of thrust:
*   **Ideal Efficiency ($96\text{ mm}$)**: **$73.5\%$** (Real-world efficiency: **$\approx 40\%$** due to nozzle drag and tip losses).
*   **Ideal Efficiency ($230\text{ mm}$)**: **$92.6\%$** (Real-world efficiency: **$\approx 65\%$** due to optimized blade geometry and low slip).
...
*   Target RPM at $4\text{ knots}$ (with a $6\text{-inch}$ pitch propeller): $\approx 1,200\text{–}1,500\text{ RPM}$ under load.
*   Operating Voltage: $48\text{V}$ nominal.
*   Required Motor KV: To avoid using complex, high-wear gearboxes or belt drives, the motor must be **directly driven** by selecting a low-KV outrunner motor (**$50\text{–}100\text{ KV}$** range, e.g., a brushless gimbal or skateboard motor).
...
1.  **Stator Selection**: Select a $50\text{mm}$ outrunner stator with thick laminations. Remove any standard paper insulation and clean with isopropyl alcohol.
2.  **Epoxy Selection**: Use **MG Chemicals 832TC** thermally conductive epoxy (thermal conductivity: $1.44\text{ W/(m}\cdot\text{K)}$). This provides high electrical resistance while ensuring heat from the windings is conducted directly to the surrounding seawater.
3.  **Vacuum Degassing**: Mix the two-part epoxy and place it in a vacuum chamber at $-29\text{ in Hg}$ for 10 minutes to remove all air bubbles.
4.  **Mold Casting**: Place the stator in a custom 3D-printed silicone mold. Pour the degassed epoxy over the coils, ensuring it fully saturates the gaps between the teeth. Cure at $65^\circ\text{C}$ for 4 hours.
5.  **Windage Reduction**: Grind the cured stator surface to a smooth cylinder on a lathe, maintaining a stator-to-rotor air gap of exactly $0.5\text{ mm}$.
...
2.  **Encapsulation Sleeve**: Glue the magnets into the rotor cup using marine-grade epoxy. Apply a thin outer wrap of **$0.2\text{mm}$ carbon-fiber prepreg tape** or press-fit a **$0.15\text{mm}$ Grade 2 Titanium sleeve** over the magnets. This creates a mechanical barrier preventing magnet swelling or delamination under water pressure.
...
*   **Viscous Shear Losses**: Because water is $\approx 800\times$ denser and $\approx 55\times$ more viscous than air, rotating a flooded rotor cup in water creates significant shear-layer drag. The viscous shear stress ($\tau$) in the stator-rotor air gap is modeled as:
    $$\tau = \mu \frac{\omega R}{g}$$
    where $\mu$ is the dynamic viscosity of water ($1.002 \times 10^{-3}\text{ Pa}\cdot\text{s}$ at $20^\circ\text{C}$), $\omega$ is angular velocity (rad/s), $R$ is rotor radius, and $g$ is the radial air gap.
*   **Power Loss Equation**: The total shear power loss ($P_{shear}$) inside the air gap of length $L$ is:
    $$P_{shear} = \frac{2\pi \mu L R^3 \omega^2}{g}$$
*   **Air Gap Optimization**: Since shear loss scales with the cube of the radius ($R^3$) and is inversely proportional to the gap ($g$), keeping the motor diameter compact ($\le 50\text{ mm}$) and widening the radial air gap from the typical $0.3\text{ mm}$ (air spec) to **$0.8\text{ mm}$** reduces viscous drag by **$62.5\%$** while only sacrificing $\approx 12\%$ of the motor torque constant ($K_t$).

### 3.6 ESC Firmware & Commutation Tuning (FOC vs. Trapezoidal)
*   **Field Oriented Control (FOC)**: Standard trapezoidal ESCs struggle to start a flooded outrunner due to the heavy startup damping of water. A VESC or AM32 ESC operating in sensorless FOC mode is required. FOC applies sinusoidal currents to track the rotor position down to $0\text{ RPM}$, providing the high starting torque needed to break the propeller loose.
*   **ESC Parametric Tuning**:
    ...
    *   *Minimum ERPM*: Set the minimum electrical RPM to $150\text{ ERPM}$ to prevent commutation stalling at low speeds.
...
*   **Sacrificial Anode**: A **$50\text{ g}$ ring-type zinc anode** must be clamped onto the exposed portion of the stainless steel propeller shaft behind the rotor cup. The zinc will corrode preferentially to protect the structural aluminum and steel components.
...
*   **Magnets**: High-coercivity $10\text{ mm} \times 10\text{ mm} \times 3\text{ mm}$ N52 block magnets.
*   **Torque Slip Limit**: Designed for a peak torque of **$1.8\text{ N}\cdot\text{m}$**. 
...
*   **Wall Thickness**: $1.5\text{ mm}$ at the coupling interface. Delrin provides excellent structural strength under hydrostatic pressure up to $1\text{ bar}$ (10 meters depth) while keeping the magnetic gap (including water clearances) to $\le 2.5\text{ mm}$.
...
*   **Wet-Side Shaft**: $6\text{ mm}$ ground 316 stainless steel shaft.
...
*   **Axial Pull Force**: The 12-pole N52 magnetic array creates a continuous axial attraction force ($F_{axial}$) pulling the external wet hub toward the dry Delrin cap. At a $2.5\text{ mm}$ coupling gap, this pull force is $\approx 120\text{ N}$ ($27\text{ lbs}$).
*   **Thrust Washer Integration**: Standard radial bearings wear out rapidly under high axial thrust. A **silicon nitride ($Si_3N_4$) ceramic thrust ball bearing** or a **glass-filled PTFE thrust washer** must be placed directly between the external coupling hub and the face of the Delrin cap to absorb the axial preload with minimal friction coefficient ($\mu \le 0.04$).
...
*   **Non-Conductive Materials**: The barrier cap must be machined from a high-performance polymer with zero electrical conductivity:
    *   *Baseline*: Delrin (POM-C) for up to $10\text{ m}$ depth (low cost, easy to machine).
    *   *High-Performance*: PEEK (Polyetheretherketone) for up to $50\text{ m}$ depth (extreme tensile strength and thermal stability up to $150^\circ\text{C}$).
*   *Resistivity*: Avoid any aluminum or steel caps, which would lose up to $25\text{ W}$ in heat and demagnetize the neodymium magnets (Curie temperature degradation starts at $80^\circ\text{C}$ for N52).
...
*   **O-Ring Sizing**: Nitrile (NBR-70) O-rings, AS568-152 standard size (cross-section $d_2 = 2.62\text{ mm}$, ID = $82.2\text{ mm}$).
*   **Gland Sizing & Compression**:
    *   *Radial Compression*: Machine the cap gland to a depth of $2.02\text{ mm}$, providing exactly $23\%$ radial compression.
    *   *Groove Width*: Machine the groove width to $3.60\text{ mm}$ to allow for O-ring volumetric expansion under hydrostatic pressure without extrusion.
...
| **Silt & Sand Tolerance**| **Low**: Sand particles can enter the $0.5\text{mm}$ air gap, causing friction/jamming. | **High**: The dry motor is isolated; external shaft has large polymer clearances. |
...
1.  **Pot the Stator**: Set up the stator in the 3D-printed silicone mold. Pour mixed MG Chemicals 832TC epoxy. Cure in a warm enclosure at $65^\circ\text{C}$ for 4 hours.
2.  **Coat the Rotor**: Apply a $0.1\text{ mm}$ layer of marine epoxy to the rotor cup interior. Press-fit the $0.15\text{ mm}$ titanium sleeve over the N52SH magnets using a hydraulic vice.
...
1.  **Machine the Barrier**: Machine the Delrin end cap, turning the coupling face down to a wall thickness of $1.5\text{ mm}$.
...
3.  **Assemble Wet Side**: Press the Igus xiros polymer bearings into the wet-side bracket. Install the external magnet cylinder onto the $6\text{mm}$ stainless steel shaft.
...
| **Full Ceramic Bearings** | 2 | ZrO2 Open Bearings ($8\text{mm} \times 16\text{mm} \times 5\text{mm}$) | \$32 |
| **Igus Polymer Bearings** | 2 | Igus xiros ($6\text{mm} \times 15\text{mm} \times 5\text{mm}$) | \$18 |
| **Neodymium Magnets** | 24 | N52 Block Magnets ($10\text{mm} \times 10\text{mm} \times 3\text{mm}$) | \$24 |
| **Titanium Sleeve** | 1 | Machined Ti-Grade 2 Foil ($0.15\text{mm}$ thickness) | \$15 |
| **Propeller Shaft** | 1 | 316 Stainless Steel Ground Shaft ($6\text{mm} \times 100\text{mm}$) | \$8 |
